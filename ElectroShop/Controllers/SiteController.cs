    ...
    public class SiteController : Controller
    {
        ...
        public ActionResult ProductDetail(String slug)
        {
            ...
            ViewBag.listother = db.Products
                .Where(m => m.Status == 1 && listcatid.Contains(m.CateID) && m.ID != model.ID)
                .OrderByDescending(m => m.Created_at)
                .Take(12)
                .ToList();
            // sản phẩm mới nhập
            ViewBag.news = db.Products
                .Where(m => m.Status == 1 && m.ID != model.ID)
                .OrderByDescending(m => m.Created_at)
                .Take(4)
                .ToList();

            // Pass the maximum available quantity to the view
            ViewBag.MaxQuantity = model.Quantity;
